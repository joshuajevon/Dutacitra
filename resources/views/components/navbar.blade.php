    ...
    <div class="flex justify-between items-center py-3">
        ...
        <span class="hidden xl:flex justify-center items-center gap-16">
            <span class="flex justify-center items-center gap-8 sm:gap-16 md:gap-20">
                ...
                <a href="{{ route('project', ['lang' => $lang]) }}" class="@if($page === 'project') font-extrabold @endif">Project</a>

                <a href="{{ route('about', ['lang' => $lang]) }}" class="@if($page === 'about') font-extrabold @endif">About Us</a>

                <a href="{{ route('contact', ['lang' => $lang]) }}" class="@if($page === 'contact') font-extrabold @endif border border-custom-white rounded-full px-6 py-1 hover:bg-custom-white hover:text-custom-darker-blue transition">Contact</a>
...
<nav id="mobile-nav-links" class="c-container pb-2 @if($bgColor === 'blue') bg-custom-darker-blue @elseif ($bgColor === 'transparent') bg-transparent @endif hidden text-custom-white text-subparagraph absolute top-16 left-0 w-full z-50">
    <a href="{{ route('home', ['lang' => $lang]) }}" class="@if($page === 'home') font-extrabold @endif block py-3.5">Home</a>

    <a href="{{ route('product', ['lang' => $lang]) }}" class="@if($page === 'product') font-extrabold @endif block py-3.5">Product</a>

    <a href="{{ route('project', ['lang' => $lang]) }}" class="@if($page === 'project') font-extrabold @endif block py-3.5">Project</a>

    <a href="{{ route('about', ['lang' => $lang]) }}" class="@if($page === 'about') font-extrabold @endif block py-3.5">About Us</a>

    <a href="{{ route('contact', ['lang' => $lang]) }}" class="@if($page === 'contact') font-extrabold @endif block py-3.5">Contact</a>
